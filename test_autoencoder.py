...
def loadfile():
    ...
    for i in range(21, 74):
        v = np.loadtxt('/home/tansinjahan/Desktop/cudaTest/volume_shape/MyTestFile' + str(i) + '.txt')
        image_matrix = np.reshape(v, (32, 32, 32)).astype(np.float32)
        input_file = np.append(input_file, image_matrix)

    input_file = np.reshape(input_file, (53, 32 * 32 * 32))
    print("This is the shape of input for 53 shape", input_file.shape)
    return input_file

def next_batch(img_array,batch_size):
